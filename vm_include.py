...
def connectToHost(host, host_user, host_pw):
    ...
    try:
        ...
        print
        "Cannot connect to host: " + host + " error message: " + e


def createGuest(host_con, guest_dc, guest_host, guest_name, guest_ver, guest_mem, guest_cpu, guest_iso, iso_ds, guest_os,
                guest_disk_gb, guest_ds, guest_network1, guest_network2, guest_enterbios):
    ...
    cr_mors = host_con._retrieve_properties_traversal(property_names=['name', 'host'], from_node=hf_mor,
    ...
    if guest_network2:
        net_name2 = guest_network2
    else:
        for net in config_target.Network:
            if net.Network.Accessible:
                net_name2 = net.Network.Name
                # get ds
    ds_target = None
    ...
    if not ds_target:
        host_con.disconnect()
        return "Cannot find datastore: " + iso_ds
    iso_vol_name = "[%s]" % iso_ds
    ...
    if ide_ctlr:
        ...
        cd_device_backing.set_element_fileName("%s %s" % (iso_vol_name, guest_iso))
    ...
    nic_spec2 = config.new_deviceChange()
    if net_name2:
        nic_spec2.set_element_operation("add")
        nic_ctlr = VI.ns0.VirtualVmxnet3_Def("nic_ctlr").pyclass()
        nic_backing = VI.ns0.VirtualEthernetCardNetworkBackingInfo_Def("nic_backing").pyclass()
        nic_backing.set_element_deviceName(net_name2)
        nic_ctlr.set_element_addressType("generated")
        nic_ctlr.set_element_backing(nic_backing)
        nic_ctlr.set_element_key(4)
        nic_spec2.set_element_device(nic_ctlr)
        devices.append(nic_spec2)
